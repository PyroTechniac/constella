use std::{fmt::{self, Binary, Debug, Display, Formatter, LowerHex, Octal, UpperHex}, marker::PhantomData};
...
mod tests {
    ...
    #[test]
    fn native_endian() {
        fn native_endian<T: ByteOrderType>() {
            for _ in 0..1024 {
        ...
        call_for_all_types!(native_endian, NativeEndian);
    }

    #[test]
    fn non_native_endian() {
        fn non_native_endian<T: ByteOrderType>() {
            for _ in 0..1024 {
                let native = T::Native::rand();
                let mut bytes = T::ByteArray::default();
                bytes_of_mut(&mut bytes).copy_from_slice(bytes_of(&native));
        ...
        call_for_all_types!(non_native_endian, NonNativeEndian);
    }
}